...
class ShadowLinkStorageModeSyncTest(ShadowLinkTestBase):
    ...
    alias means (different cloud_storage_default_mode values).

    The sync transports the describe output, where both tiered variants
    display as 'tiered'; the read-only redpanda.storage.mode.version
    ...
    def __init__(self, test_context: TestContext, *args: Any, **kwargs: Any):
        ...
        super().__init__(
            ...
            extra_rp_conf={
                ...
                "cloud_storage_default_mode": target_default,
            },
            secondary_cluster_args=SecondaryClusterArgs(
                ...
                extra_rp_conf={
                    ...
                    "enable_cluster_metadata_upload_loop": False,
                    "cloud_storage_default_mode": source_default,
                },
            ),
            *args,
            **kwargs,
    ...
    def _target_storage_mode(self, topic: str) -> tuple[str | None, str | None]:
        ...
        version = configs.get(TopicSpec.PROPERTY_STORAGE_MODE_VERSION)
        return (
    ...
    def test_storage_mode_sync_across_defaults(
        ...
        # topic -> (create config, expected version on the target;
        # None = the version property must be absent)
        cases: dict[str, tuple[dict[str, str], str | None]] = {
            "sync-cloud": (
            ...
            "sync-tiered-v1": (
                TopicSpec.storage_mode_config(TopicSpec.STORAGE_MODE_VERSION_TIERED_V1),
                TopicSpec.STORAGE_MODE_VERSION_TIERED_V1,
            ),
            "sync-tiered-v2": (
                TopicSpec.storage_mode_config(TopicSpec.STORAGE_MODE_VERSION_TIERED_V2),
        ...
        for name, (_, expected_version) in cases.items():
            expected_mode = (
        ...
        if source_default == "tiered_v2":
            ...
            def converted() -> bool:
                ...
                return (
                    ...
                    and version == TopicSpec.STORAGE_MODE_VERSION_TIERED_V2
                )
