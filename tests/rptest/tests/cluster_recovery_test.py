    ...
    def test_cloud_topics_recovery_with_bootstrap_params(self):
        ...
        for partition in range(self.partition_count):
            offsets = self._wait_for_metastore_sync(
                ...
                self.topic_name,
                partition,
                expected_min_next_offset=1,  # At least some data
            )
            self.logger.info(
        ...
        # Verify local partition state before recovery
        for part in rpk.describe_topic(self.topic_name):
            self.logger.info(
                f"Before recovery - partition {part.id}: "
                f"start_offset={part.start_offset}, "
                f"high_watermark={part.high_watermark}"
            )

        # Step 4: Wait for controller metadata to be uploaded
        self.logger.info("Waiting for controller metadata upload")
        self.redpanda.wait_for_controller_snapshot(self.redpanda.nodes[0])

        # Step 5: Perform cluster recovery
        self.logger.info("Performing cluster recovery")
        self._perform_cluster_recovery(rpk)

        # Step 6: Verify topic was restored
        ...
        # Step 7: Produce new data to the recovered topic
        self.logger.info("Producing new data to recovered topic")
        self._produce_data(msg_count=200)

        # Step 8: Wait for new data to be reconciled to metastore
        # This verifies that the recovered topic with bootstrap params
        # can accept new writes and reconcile them correctly
        self.logger.info("Waiting for new data to reconcile to metastore")
        for partition in range(self.partition_count):
            # Wait for next_offset to increase, indicating new data was reconciled
            offsets = self._wait_for_metastore_sync(
                admin,
                self.topic_name,
                partition,
                expected_min_next_offset=1,  # At least some data after recovery
            )
            self.logger.info(
                f"After recovery - metastore offsets for partition {partition}: "
                f"start_offset={offsets.start_offset}, "
                f"next_offset={offsets.next_offset}"
            )

        # Step 9: Verify partition state after recovery
        for part in rpk.describe_topic(self.topic_name):
            self.logger.info(
                f"After recovery - partition {part.id}: "
                f"start_offset={part.start_offset}, "
                f"high_watermark={part.high_watermark}"
            )
