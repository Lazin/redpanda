...
allowed_local_start_offset (ALSO) is a kafka-offset floor set by L1 compaction:
...
does not run, so ALSO stays unset; the cap on local data comes from the storage
...
- Switching cleanup.policy to compact causes L1 compaction to run, which
  advances ALSO; ctp_stm housekeeping then evicts up to the new floor.
...
class TieredCloudLocalRetentionTest(EndToEndCloudTopicsBase):
    ...
    that enabling compaction advances ALSO via L1 compaction, and that reads
    ...
    def __init__(self, test_context: TestContext):
        extra_rp_conf = {
            ...
            # ALSO advance within the test timeout.
    ...
    def test_local_data_grows_in_tiered_cloud(self):
        ...
        L1 compaction does not run for cleanup.policy=delete, so ALSO stays
    ...
    def test_space_manager_reclaim_under_pressure(self):
        ...
        ceiling and is unaffected by ALSO or by the local target, so
    ...
    def test_read_below_local_start_pivots_to_l1(self):
        ...
        If the pivot keyed on LRO or on allowed_local_start_offset (which is
