...
def _verify_file_layout(baseline_per_host,
    ...
    for ntp, orig_ntp_size in baseline_ntps.items():
        ...
        assert delta <= default_log_segment_size, f"NTP {ntp} the restored partition is too small {rest_ntp_size}. The original is {orig_ntp_size} bytes which {delta} bytes larger."
