...
from rptest.archival.s3_client import S3Client, ClientError
...
class BaseCase:
    ...
    def restore_redpanda(self, baseline, controller_checksums):
        ...
        for topic in self.expected_recovered_topics:
            for path, manifest in topic_manifests:
                if manifest['topic'] == topic.name:
                    self._restore_topic(path, manifest)
    ...
    def _restore_topic(self, path, manifest, overrides={}):
        ...
        self.logger.info(
            f"Restore topic called. Path: {path}, topic-manifest: {manifest}")
...
class MissingTopicManifest(BaseCase):
    ...
    def __init__(self, s3_client, kafka_tools, rpk_client, s3_bucket, logger):
        super(MissingTopicManifest, self).__init__(s3_client, kafka_tools, rpk_client,
                                                   s3_bucket, logger)
...
class MissingSegment(BaseCase):
    ...
    def validate_cluster(self, baseline, restored):
        ...
        _find_checksum_matches(baseline, restored, expected_topics,
                               self.logger)
        for topic in self.topics:
...
class MovedPartitionRestore(BaseCase):
    """Check situation when the partition is move
    rom one node to the other. In this case it will be asigned new revision id.
...
class CascadingRestore(BaseCase):
    ...
    def _relocate_oldest_segment(self):
        ...
        for id in manifests:
            ...
            revision = int(obj['revision'] * 2)
            ntp = NTP(ns=ns, topic=topic, partition=partition)
            new_name = _gen_segment_path(ntp, revision, target_name)
            self.logger.info(
...
class TopicRecoveryTest(RedpandaTest):
    ...
    def __init__(self, test_context):
        ...
        if self.real_thing:
            ...
            self.s3_bucket = TopicRecoveryTest.MINIO_BUCKET_NAME
            ...
            extra_rp_conf = dict(
                ...
                cloud_storage_bucket=TopicRecoveryTest.MINIO_BUCKET_NAME,
    ...
    def test_missing_topic_manifest(self):
        ...
        in old cluster the empty topic should be created. We should be able
        to produce to the topic."""
        test_case = MissingTopicManifest(self.s3_client, self.kafka_tools, self.rpk,
                                         self.s3_bucket, self.logger)
        self.do_run(test_case)
    ...
    def test_missing_segment(self):
        ...
        bucket."""
        test_case = MissingSegment(self.s3_client, self.kafka_tools,
                                   self.rpk, self.s3_bucket, self.logger)
        self.do_run(test_case)
