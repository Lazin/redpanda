...
SegmentMeta = collections.namedtuple(
    'SegmentMeta', ('base_offset', 'base_timestamp', 'last_offset', 'max_timestamp', 'num_configs', 'total_size'))
...
SegmentPathComponents = collections.namedtuple(
    'SegmentPathComponents', ('ns', 'topic', 'part_id', 'rev_id', 'base_offset', 'segment_term', 'archiver_term'))
...
def parse_segment_name(sname):
    ...
    if tail.endswith('.log'):
        # old-style name
        return SegmentPathComponents(ns, tn, int(pid), int(rev), int(boff), int(term), None)
    else:
        _, _, aterm = tail.split('.')
        return SegmentPathComponents(ns, tn, int(pid), int(rev), int(boff), int(term), int(aterm))


def make_segment_name(offset, term):
    """Create segment name that can be used as a key in
    the manifest. Archiver term is not added to the name."""
...
def main():
    ...
    if options.op == "gencmd":
        ...
        for tm in topic_manifests:
            ...
            topic = tmobj['topic']
            cmd_all = f"rpk topic create {topic} -r {rf} -p {parts} -c cleanup.policy=delete -c retention.ms=100000000000 -c retention.bytes=999999999999 -c redpanda.remote.recovery=true"
            print(cmd_all)
            print("sleep 5")
        print("\nDone")
        ...
        for tm in topic_manifests:
            # if 'kyc-nexus_sources_mz-sink-consistency' not in tm:
            #     continue
            data = client.get_object_data(options.bucket, tm)
            tmobj = json.loads(data)
            ns = tmobj['namespace']
            ...
            for part_id in range(0, partition_count):
                ...
                for segment_name in segments:
                    print(f"looking at {segment_name}")
                ...
                if len(partition_segments) == 0:
                    continue
                last_rev = partition_segments[-1][0]
                partition_segments = filter(lambda x: x[0] == last_rev, partition_segments)
                for rev, base_offset, term, aterm, path in partition_segments:
                    segment_metadata = get_segment_meta(client, options.bucket, path)
                    _, base_timestamp, last_offset, max_timestamp, num_configs, size_bytes = segment_metadata
