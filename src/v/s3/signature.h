...
using aws_region_name = named_type<std::string, struct s3_aws_region_name>;
using public_key_str = named_type<std::string, struct s3_public_key_str>;
using private_key_str = named_type<std::string, struct s3_private_key_str>;
...
class time_source {
    enum {
        formatted_date_len = 9,      // format is 20201231\0
        formatted_datetime_len = 17, // format is 20201231T123100Z\0
    };
...
public:
    ...
    time_source(time_source&&) = default;
    time_source& operator=(time_source&&) = default;
    ~time_source() = default;
    ...
    /// Return formatted date in ISO8601 format
    std::string format_datetime() const;
...
private:
    ...
    std::function<timestamp()> _gettime;
};
...
public:
    ...
    static result<std::string> gen_sig_key(
      std::string_view key,
    ...
    /// requirements)
    static std::string sha256_hexdigest(std::string_view payload);
