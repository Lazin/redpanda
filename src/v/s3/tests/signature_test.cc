...
SEASTAR_THREAD_TEST_CASE(test_signature_computation_1) {
    ...
    header.insert(http::client::field::range, "bytes=0-9");
    ...
    BOOST_REQUIRE_EQUAL(
      header.at(http::client::field::authorization), expected);
}
...
SEASTAR_THREAD_TEST_CASE(test_signature_computation_2) {
    ...
    header.insert(http::client::field::host, host);
    header.insert(http::client::field::date, "Fri, 24 May 2013 00:00:00 GMT");
...
SEASTAR_THREAD_TEST_CASE(test_gnutls) {
    ...
    std::memcpy(result.data(), digest.value().data(), 32);
    BOOST_REQUIRE_EQUAL(
