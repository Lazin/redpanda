...
ss::future<cloud_storage::download_result> ntp_archiver::download_manifest() {
    ...
    vlog(archival_log.debug, "Downloading manifest for {}", _ntp);
    co_return co_await _remote.download_manifest(
      _bucket, _manifest, max_backoff_manifest);
}
...
ntp_archiver::upload_next_candidates(storage::log_manager& lm) {
    ...
    total.num_failed = std::count(
      begin(results), end(results), cloud_storage::upload_result::timedout);
