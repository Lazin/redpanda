...
private:
    ...
    ss::future<aggregated_upload_result> upload_segment_index(
      ...
      segment_upload_candidate_ptr upload,
    ...
    segment_index_object_names(
      ...
      const segment_upload_candidate_ptr& upl) {
        auto path = part->manifest().generate_segment_path(upl->metadata);
    ...
    ss::future<aggregated_upload_result> upload_candidate(
      ...
      segment_upload_candidate_ptr upl) {
        ...
        if (upl->tx.size() > 0) {
            ...
            uploads.emplace_back(
              _api->upload_manifest(_bucket, *txm, workflow_rtc)
    ...
    ss::future<aggregated_upload_result> upload_segment(
      ...
      segment_upload_candidate_ptr candidate,
