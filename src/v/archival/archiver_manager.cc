...
std::ostream& operator<<(
  std::ostream& o, const managed_partition_event_base<event, Derived>& e) {
    fmt::print(o, "[{}..{}]", event, static_cast<const auto_fmt<Derived>>(e));
    return o;
}
...
public:
    ...
    struct st_passive : state<managed_partition_state_t::passive> {
        std::optional<ss::promise<>> trigger;
        void clear() noexcept { trigger = std::nullopt; }
    };
    ...
    struct st_starting_async : state<managed_partition_state_t::starting> {
        // ntp_archiver::start operation that runs in the background
    ...
    struct tr_passive_to_starting {
        void operator()(
          ...
          st_starting_async& next) {
            ...
            ssx::background
              = fsm.bg_create_and_start_archiver(new_leadership.term)
    ...
    struct tr_starting_to_passive {
        void operator()(
          ...
          st_passive& next) {
            ...
            prev.clear();
            if (next.trigger) {
                next.trigger->set_exception(f.error);
            }
        }
    };
    ...
    struct tr_active_to_stopping {
        ...
        void operator()(
          ...
          state_machine_t& fsm,
          st_active& active,
          st_stopping_async& stopping) {
            vlog(fsm._ctxlog.info, "stopping ntp_archiver");
            stopping.reset_units = fsm._part->get_archiver_reset_units();
            ...
            ssx::background = fsm.bg_stop_and_dispose_archiver(active.archiver)
                                .then_wrapped(completion)
                                .finally([arch = active.archiver] {});
            active.clear();
        }
    };
...
private:
    ...
    bg_create_and_start_archiver(model::term_id expected_term) noexcept {
    ...
    ss::future<> bg_stop_and_dispose_archiver(
...
struct managed_partition : public managed_partition_fsm::state_machine_t {
    ...
    ss::future<> stop() {
        ...
        st.trigger = ss::promise<>();
        process_event(ev_shutdown{});
        co_await st.trigger->get_future();
    }
...
private:
    ...
    // Top level archiver_manager gate holder
...
public:
    ...
    ss::future<> stop() {
        ...
        vlog(
          _logger.info,
          "Stopping archiver_manager, gate count {}",
          _gate.get_count());
        _managed.clear();
    }
