...
public:
    ...
    std::optional<iobuf> read_offset_range(inclusive_offset_range range) {
        ...
        auto last_data_offset = upl_res.value()->get_meta().last_real_offset;
        ...
        BOOST_REQUIRE_LE(range.last, last_data_offset);
    ...
    auto load_log_segment_concat(inclusive_offset_range range) {
        ...
        for (auto& s : get_partition_log()->segments()) {
            auto o = s->offsets();
            if (
              o.base_offset >= range.base && o.committed_offset <= range.last) {
                // s inside the range: r[...........]r
                //                         o[...]o
                segments.push_back(s);
            } else if (
              range.base >= o.base_offset && range.last <= o.committed_offset) {
                // range inside the segment: o[...........]o
                //                               r[...]r
                segments.push_back(s);
            } else if (
              range.base <= o.base_offset && range.last >= o.base_offset) {
                // begin overlaps: r[.........]r
                //                      o[........]o
                segments.push_back(s);
            } else if (
              o.base_offset < range.base && o.committed_offset >= range.base) {
                // begin overlaps: o[........]o
                //                      r[........]r
                segments.push_back(s);
            }
...
FIXTURE_TEST(
  test_segment_upload_full_non_compacted, async_data_uploader_fixture) {
...
FIXTURE_TEST(test_segment_upload_full_compacted, async_data_uploader_fixture) {
    random_records_generator generator;
...
FIXTURE_TEST(
  test_segment_upload_partial_non_compacted, async_data_uploader_fixture) {
    random_records_generator generator;
    generate_partition(1000, generator);
...
struct fuzz_test_case {
    ...
    // expected timestamps
    model::timestamp expected_base_ts;
...
inline void get_random_test_cases_impl(
  ...
  Fn pick_offsets) {
    ...
    for (int i = 0; i < num_results; i++) {
        ...
        std::optional<model::timestamp> ts_base = std::nullopt,
                                        ts_last = std::nullopt;
        for (auto j = ix_first; j < ix.size(); j++) {
            if (map.batch_types.at(j) == model::record_batch_type::raft_data) {
                // record timestamp
                ts_base = map.timestamps.at(j);
                vlog(test_log.debug, "Found base timestamp: {}", ts_base);
                break;
            }
        }
        if (ts_base.has_value() == false) {
            vlog(
              test_log.debug, "Test case #{} base timestamp can't be found", i);
        }
        for (auto j = ix_second; j < ix.size(); j++) {
            if (map.batch_types.at(j) == model::record_batch_type::raft_data) {
                // record timestamp
                ts_last = map.timestamps.at(j);
                vlog(test_log.debug, "Found last timestamp: {}", ts_last);
                break;
            }
        }
        if (ts_last.has_value() == false) {
            vlog(
        ...
        vlog(
          test_log.debug,
          "Test case #{}/{}: offsets: {}-{}, timestamps: {}-{}, upload is "
          "possible: {}",
          i,
          num_results,
          tc.base,
...
FIXTURE_TEST(
  test_segment_upload_random_not_compacted, async_data_uploader_fixture) {
    vlog(
      test_log.info,
...
FIXTURE_TEST(
  test_segment_upload_random_compacted, async_data_uploader_fixture) {
    vlog(
      test_log.info,
      "Seed used for the test: {}",
      random_generators::internal::seed);
