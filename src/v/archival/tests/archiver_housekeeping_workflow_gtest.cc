...
TEST(
  archiver_housekeeping_workflow_test,
  test_stm_housekeeping_apply_retention_shutdown) {
    // Test case: the workflow is doing housekeeping of the
    // STM region of the log and apply_stm_retention method
...
TEST(
  archiver_housekeeping_workflow_test,
  test_stm_housekeeping_apply_retention_shutdown_exception) {
    // Test case: the workflow is doing housekeeping of the
    // STM region of the log and apply_stm_retention method
...
TEST(
  archiver_housekeeping_workflow_test,
  test_archive_housekeeping_apply_retention_errc) {
    // Test case: the workflow is doing housekeeping of the
    // archive region of the log and apply_stm_retention method returns
    // an error.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(
  archiver_housekeeping_workflow_test,
  test_archive_housekeeping_apply_retention_exception) {
    // Test case: the workflow is doing housekeeping of the
    // archive region of the log and apply_stm_retention method
    // throws an exception.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(
  archiver_housekeeping_workflow_test,
  test_archive_housekeeping_apply_retention_shutdown) {
    // Test case: the workflow is doing housekeeping of the
    // archive region of the log and apply_stm_retention method
    // return a shutdown error.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(
  archiver_housekeeping_workflow_test,
  test_archive_housekeeping_apply_retention_shutdown_exception) {
    // Test case: the workflow is doing housekeeping of the
    // archive region of the log and apply_stm_retention method
    // throws a shutdown exception.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(archiver_housekeeping_workflow_test, test_stm_housekeeping_gc_errc) {
    // Test case: the workflow is doing housekeeping of the
    // STM region of the log and GC method returns
    // an error.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();

    {
        ...
        rm_api->expect_maybe_suspend_housekeeping(
          suspend_housekeeping_request{
            .ntp = ntp,
            // Set to true because 'apply_stm_retention' call
            // successfully advanced start_offset.
            .manifest_dirty = true,
            .errc = error_outcome::timed_out,
          },
          make_error_code(error_outcome::shutting_down));
...
TEST(archiver_housekeeping_workflow_test, test_stm_housekeeping_gc_exception) {
    // Test case: the workflow is doing housekeeping of the
    // STM region of the log and GC method throws the exception.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(archiver_housekeeping_workflow_test, test_stm_housekeeping_gc_shutdown) {
    // Test case: the workflow is doing housekeeping of the
    // STM region of the log and GC method which returns shutdown error
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(
  archiver_housekeeping_workflow_test,
  test_stm_housekeeping_gc_shutdown_exception) {
    // Test case: the workflow is doing housekeeping of the
    // STM region of the log and GC method which throws shutdown exception
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(archiver_housekeeping_workflow_test, test_archive_housekeeping_gc_errc) {
    // Test case: the workflow is doing housekeeping of the
    // archive region of the log and archive GC method returns
    // an error.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();

    {
        ...
        rm_api->expect_maybe_suspend_housekeeping(
          suspend_housekeeping_request{
            .ntp = ntp,
            // apply_archive_retention is successful
            // so the manifest is updated
            .manifest_dirty = true,
            .errc = error_outcome::timed_out,
          },
          make_error_code(error_outcome::shutting_down));
...
TEST(
  archiver_housekeeping_workflow_test, test_archive_housekeeping_gc_exception) {
    // Test case: the workflow is doing housekeeping of the
    // archive region of the log and archive GC method throws.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(
  archiver_housekeeping_workflow_test, test_archive_housekeeping_gc_shutdown) {
    // Test case: the workflow is doing housekeeping of the
    // archive region of the log and archive GC method returns shutdown errc.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(
  archiver_housekeeping_workflow_test,
  test_archive_housekeeping_gc_shutdown_exception) {
    // Test case: the workflow is doing housekeeping of the
    // archive region of the log and archive GC method throw any shutdown error.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(archiver_housekeeping_workflow_test, test_archive_housekeeping_gc2_errc) {
    // Test case: the workflow is doing housekeeping of the
    // archive region of the log and STM GC method returns
    // an error.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(
  archiver_housekeeping_workflow_test,
  test_archive_housekeeping_gc2_exception) {
    // Test case: the workflow is doing housekeeping of the
    // archive region of the log and STM GC method throws.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(
  archiver_housekeeping_workflow_test, test_archive_housekeeping_gc2_shutdown) {
    // Test case: the workflow is doing housekeeping of the
    // archive region of the log and STM GC method returns shutdown error.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
...
TEST(
  archiver_housekeeping_workflow_test,
  test_archive_housekeeping_gc2_shutdown_exception) {
    // Test case: the workflow is doing housekeeping of the
    // archive region of the log and STM GC method throw shutdown exception.
    auto [wf, rm_api, op_api, cfg] = setup_housekeeping_test_suite();
