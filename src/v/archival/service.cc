...
ss::future<> scheduler_service_impl::reconcile_archivers() {
    ...
    /*TODO: remove*/ for (const auto& s : snapshot) {
        vlog(archival_log.debug, "Snapshot ntp: {}", s.path());
    }
    /*TODO: remove*/ for (const auto& s : _queue) {
        vlog(archival_log.debug, "Queue ntp: {}", s.first.path());
    }
    ...
    _queue.copy_if(
      std::back_inserter(to_remove), [&snapshot, &pm](const model::ntp& ntp) {
          ...
          /*TODO: remove*/ if (p) {
              t = p->is_leader() ? "leader" : "not-leader";
          }
          /*TODO: remove*/ vlog(
            archival_log.debug, "...queue check for {} - {}", ntp.path(), t);
    ...
    std::copy_if(
      ...
      [this, &pm](const model::ntp& ntp) {
          ...
          /*TODO: remove*/ vlog(
            archival_log.debug, "...snapshot check for {} - {}", ntp.path(), t);
