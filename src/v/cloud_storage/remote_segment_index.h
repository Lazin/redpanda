...
public:
    ...
    /// Find index entry that corresponds to the redpanda offset
    ///
    /// The returned value has rp_offset equal or less than upper_bound
    /// If the nullopt is returned then all elements are smaller than
    /// lower_bound.
    std::optional<find_result>
    lower_bound_rp_offset(model::offset upper_bound); // TODO: rename

    /// Find index entry that corresponds to the kafka offset
    ///
    /// The returned value has kaf_offset equal or less than upper_bound
    /// If the nullopt is returned then all elements are smaller than
    /// lower_bound.
    std::optional<find_result>
    lower_bound_kaf_offset(model::offset upper_bound); // TODO: rename
