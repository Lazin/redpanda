...
public:
    ...
    remote_partition(const manifest& m, remote& api, s3::bucket_name bucket);
    ...
    model::offset first_uploaded_offset() const {
        ...
        for (const auto& m : _manifest) {
            starting_offset = std::min(
              starting_offset, m.second.committed_offset);
        }
...
private:
    ...
    std::optional<ss::lw_shared_ptr<remote_segment>> _current;
