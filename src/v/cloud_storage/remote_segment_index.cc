...
void offset_index::add(
  model::offset rp_offset, model::offset kaf_offset, int64_t file_offset) {
    ...
    try {
        ...
        _kaf_offsets = {};
        _file_offsets = {};
        _rp_encoder = details::deltafor_encoder<int64_t>(_initial_rp);
        _kaf_encoder = details::deltafor_encoder<int64_t>(_initial_kaf);
        _file_encoder = details::deltafor_encoder<int64_t>(_initial_file_pos);
        throw;
    }
}
...
std::
  ...
  offset_index::maybe_find_offset(
    ...
    details::deltafor_decoder<int64_t> decoder(
      encoder.get_initial_value(), encoder.get_row_count(), encoder.copy());
...
void remote_segment_index_builder::consume_batch_start(
  ...
  size_t size_on_disk) {
    if (hdr.type == model::record_batch_type::raft_data) {
        ...
        _running_delta += hdr.record_count;
    }
