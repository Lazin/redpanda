...
FIXTURE_TEST(test_download_manifest, s3_imposter_fixture) { // NOLINT
    ...
    auto res = remote
                 .download_manifest(s3::bucket_name("bucket"), actual, 100ms)
                 .get0();
...
FIXTURE_TEST(test_upload_segment, s3_imposter_fixture) { // NOLINT
    ...
    auto res
      = remote
          .upload_segment(
            s3::bucket_name("bucket"), name, clen, reset_stream, m, 100ms)
          .get();
...
FIXTURE_TEST(test_download_segment, s3_imposter_fixture) { // NOLINT
    ...
    auto upl_res
      = remote.upload_segment(bucket, name, clen, reset_stream, m, 100ms).get();
    ...
    auto dnl_res
      = remote.download_segment(bucket, name, m, try_consume, 100ms).get();
