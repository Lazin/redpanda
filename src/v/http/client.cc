...
ss::future<bytes>
client::fetch(client& cl, request_header&& header, iobuf&& body) {
    return cl.make_request(std::move(header))
      .then([body = std::move(body)](request_response_t reqresp) mutable {
          auto [request, response] = std::move(reqresp);
          auto fsend
            = request->send_some(std::move(body)).then([request = request]() {
          ...
          auto frecv = fsend.then([response = response]() {
              return ss::do_with(iobuf(), [response](iobuf& result) {
                  return ss::repeat([response, &result]() {
                             ...
                             return response->recv_some()
                               .then([&result](iobuf&& buf) {
                    ...
                    .then([response, &result] {
                        ...
                        vlog(
                          ...
                          "result size :\n {}",
                          result.size_bytes());
                        return ss::make_ready_future<bytes>(
                          iobuf_to_bytes(result));
                    });
              });
          });
          return frecv;
