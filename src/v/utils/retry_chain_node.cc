...
/// Jitter
thread_local static fast_prng _retry_rand_jitter_source;
thread_local static uint32_t _logger_fiber_cnt = 0;
static constexpr size_t _max_retry_chain_depth = 8;
static constexpr uint16_t _max_initial_backoff
  = std::numeric_limits<uint16_t>::max();
static constexpr uint16_t _max_retry_count
  = std::numeric_limits<uint16_t>::max() - 1;

retry_chain_node::retry_chain_node()
  : _id(_logger_fiber_cnt++) // generate new head id
  , _backoff{0}
...
retry_chain_node::retry_chain_node(
  ...
  , _parent() {
    vassert(
      backoff.count() <= _max_initial_backoff,
...
retry_chain_node::retry_chain_node(retry_chain_node* parent)
  ...
  , _parent{parent} {
    vassert(
      get_len() < _max_retry_chain_depth, "Retry chain is too deep (max=8)");
}
...
ss::lowres_clock::duration retry_chain_node::get_backoff() {
    ...
    auto jitter = ss::lowres_clock::duration(
      _retry_rand_jitter_source() % backoff.count());
    return backoff + jitter;
}

ss::lowres_clock::duration retry_chain_node::get_poll_interval() {
    auto jitter = _retry_rand_jitter_source() % _backoff;
...
ss::sstring retry_chain_node::format() const {
    std::array<uint16_t, _max_retry_chain_depth> ids{_id};
    int ids_len = 1;
    auto next = get_parent();
    while (next) {
    ...
    int ix = 0;
    for (auto id = ids.rbegin() + (_max_retry_chain_depth - ids_len);
         id != ids.rend();
         ix++, id++) {
        if (ix == 0) {
            fmt::print(str, "[fiber{}", *id);
        } else {
            fmt::print(str, "~{}", *id);
        }
    }
    if (_deadline != ss::lowres_clock::time_point::min()) {
        ...
        // [fiber42~0~4|2|100ms]
        fmt::print(str, "|{}|{}ms]", _retry, time_budget.count());
    } else {
        // [fiber42...]
        fmt::print(str, "]");
    }
