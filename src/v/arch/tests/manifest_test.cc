...
SEASTAR_TEST_CASE(test_manifest_path) {
    return ss::async([] {
        arch::manifest m(manifest_ntp, model::revision_id(0));
...
SEASTAR_TEST_CASE(test_segment_path) {
    return ss::async([] {
        arch::manifest m(manifest_ntp, model::revision_id(0));
        auto path = m.get_remote_segment_path(
          arch::segment_name("22-11-v1.log"));
        // use pre-calculated murmur hash value from full ntp path + file name
        BOOST_REQUIRE_EQUAL(
          path, "7da10588/test-ns/test-topic/42_0/22-11-v1.log");
    });
}

SEASTAR_TEST_CASE(test_empty_manifest_update) {
    return ss::async([] {
        arch::manifest m;
...
SEASTAR_TEST_CASE(test_complete_manifest_update) {
    return ss::async([] {
        arch::manifest m;
        ...
        std::map<ss::sstring, arch::manifest::segment_meta> expected = {
          {"10-1-v1.log",
           arch::manifest::segment_meta{
...
SEASTAR_TEST_CASE(test_manifest_difference) {
    return ss::async([] {
        arch::manifest a(manifest_ntp, model::revision_id(0));
        a.add(arch::segment_name("0-0-1.log"), {});
        a.add(arch::segment_name("0-0-2.log"), {});
        a.add(arch::segment_name("0-0-3.log"), {});
        arch::manifest b(manifest_ntp, model::revision_id(0));
        b.add(arch::segment_name("0-0-1.log"), {});
        b.add(arch::segment_name("0-0-2.log"), {});
        {
        ...
        b.add(arch::segment_name("0-0-3.log"), {});
        b.add(arch::segment_name("0-0-4.log"), {});
        {
            auto c = a.difference(b);
