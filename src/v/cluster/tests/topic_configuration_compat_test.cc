...
template<>
struct adl<old::incremental_topic_updates> {
    void to(iobuf& out, old::incremental_topic_updates&& t) {
        reflection::serialize(
          out,
          old::incremental_topic_updates::version,
          t.compression,
    ...
    old::incremental_topic_updates from(iobuf_parser& in) {
        auto version = adl<int8_t>{}.from(in.peek(1));
        if (version < 0) {
            // Consume version from stream
            in.skip(1);
            vassert(
              version >= old::incremental_topic_updates::version,
        ...
        old::incremental_topic_updates updates;
        updates.compression
...
template<>
struct adl<old::topic_properties_update> {
    void to(iobuf& out, old::topic_properties_update&& r) {
        reflection::serialize(out, r.tp_ns, r.properties);
    }

    old::topic_properties_update from(iobuf_parser& parser) {
        auto tp_ns = adl<model::topic_namespace>{}.from(parser);
        old::topic_properties_update ret(std::move(tp_ns));
        ret.properties = adl<old::incremental_topic_updates>{}.from(parser);
        return ret;
    }
};
...
SEASTAR_THREAD_TEST_CASE(incremental_topic_updates_upgrade_rt_test) {
    incremental_topic_updates_roundtrip<
      old::incremental_topic_updates,
...
SEASTAR_THREAD_TEST_CASE(topic_properties_updates_upgrade_rt_test) {
    topic_properties_updates_roundtrip<
      old::topic_properties_update,
