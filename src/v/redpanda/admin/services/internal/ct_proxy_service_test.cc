...
TEST(CtProxyServiceTest, ProtoExtentMetaConversion) {
    ...
    proto_ph.set_object_id_uuid(
      std::string(
        reinterpret_cast<const char*>(meta.id.name.data()),
        meta.id.name.size()));
    ...
    EXPECT_EQ(proto_ph.get_object_id_uuid().size(), 16);
}
...
TEST(CtProxyServiceTest, PlaceholderDataValidation) {
    ...
    proto_ph.set_object_id_uuid("too_short");
    ...
    EXPECT_EQ(proto_ph.get_object_id_uuid().size(), 9);  // too short
}
...
TEST(CtProxyServiceTest, PlaceholderBatchEncoding) {
    ...
    model::record_batch_header header{
      ...
      .first_offset = model::offset(meta.base_offset()),
    ...
    auto parsed = cloud_topics::parse_placeholder_batch(batch);
...
TEST(CtProxyServiceTest, GetClusterEpochRequest) {
    ...
    req.mutable_partition()->set_topic("test-topic");
    req.mutable_partition()->set_partition(0);
...
TEST(CtProxyServiceTest, ReplicatePlaceholdersRequest) {
    ...
    // Add a placeholder
    auto* ph = req.add_placeholders();
    uuid_t test_uuid = uuid_t::create();
    ph->set_object_id_uuid(
      std::string(
        reinterpret_cast<const char*>(test_uuid.data()),
        test_uuid.size()));
...
TEST(CtProxyServiceTest, ReadPlaceholdersRequest) {
    ...
    EXPECT_TRUE(req.get_max_bytes().has_value());
    EXPECT_EQ(req.get_max_bytes().value(), 1024 * 1024);
}
...
TEST(CtProxyServiceTest, ListCloudTopicPartitionsRequest) {
    ...
    EXPECT_TRUE(req.get_topic_filter().has_value());
    EXPECT_EQ(req.get_topic_filter().value(), "test-.*");
}
