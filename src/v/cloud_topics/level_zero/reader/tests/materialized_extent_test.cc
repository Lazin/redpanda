...
TEST_F_CORO(materialized_extent_fixture, materialize_from_cloud) {
    ...
    produce_placeholders(false, 1); // false = skip cache, download from cloud
    ...
    // NOTE: The mock doesn't invoke the success_cb callback, so
    // num_cloud_reads won't be incremented
}
