...
TEST_F_CORO(level_zero_notifier_fixture, replicate_to_leader_succeeds) {
    ...
    auto res = co_await notifier.replicate_with_retries(
      leader_api, kafka::offset(42));
    co_await notifier.stop();

    ASSERT_TRUE_CORO(res.has_value());
}

// Replicating to a follower keeps hitting not_leader, so the notifier gives up
// after max_attempts and returns the error to the caller (not fire-and-forget).
