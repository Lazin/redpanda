...
/// (allowed_local_start_offset) updates to the partition that owns them.
///
/// L1 compaction (running on the maintenance shard) hands the new floor for a
/// partition to set_allowed_local_start_offset. The service resolves the
...
public:
    ...
    /// allowed_local_start_offset floor through ctp_stm_api. Returns the
    /// replication result; a partition that is not hosted on this node (or has
    /// no ctp_stm) is reported as a no-op success.
    ss::future<std::expected<void, ctp_stm_api_errc>>
    set_allowed_local_start_offset(model::ntp ntp, kafka::offset new_floor);
    ...
    ss::future<std::expected<void, ctp_stm_api_errc>>
    replicate_with_retries(ctp_stm_api& api, kafka::offset new_floor);
