...
read_debounce<Clock>::process_single_request(read_request<Clock>* req) {
    ...
    try {
        ...
        // It's not guaranteed that there are no in-flight requests
        // that target the same UUID but the probability of this is low.
        // We're waiting limited amount of time for units. It's expected
        // that the majority of GetObject requests will be fulfilled within
        // the debounce_interval.
