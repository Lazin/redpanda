...
ss::future<model::offset> ctp_stm::prefix_truncate_target() {
    ...
    //     allowed_local_start_offset floor is redundant locally.
    ...
    if (
      ...
      || ntp_cfg.is_remotely_compacted()) {
        ...
        auto off = co_await _raft->log()->adjusted_retention_offset(cfg);
        retention_target = off.value_or(model::offset::min());
    }

    // The allowed local start offset is a kafka-offset floor set by L1
    ...
    model::offset allowed_local_start_offset_log = model::offset::min();
    if (
      auto allowed_start = _state.get_allowed_local_start_offset();
        ...
        try {
            ...
            if (
              ...
              && translated != model::offset::min()) {
                allowed_local_start_offset_log = translated;
            } else {
                vlog(
                  _log.warn,
                  "[{}] allowed_local_start_offset {} could not be translated "
            ...
            vlog(
              _log.warn,
              "[{}] allowed_local_start_offset {} translation to log offset "
    ...
    auto floor = std::max(retention_target, allowed_local_start_offset_log);
