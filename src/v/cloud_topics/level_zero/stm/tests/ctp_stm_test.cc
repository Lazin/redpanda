...
TEST_F_CORO(ctp_stm_fixture, test_basic) {
    ...
    auto gc_epoch = api(node(*get_leader())).get_min_epoch();
...
TEST_F_CORO(ctp_stm_fixture, test_fencing) {
    ...
    ASSERT_EQ_CORO(projected_epoch.value(), ct::cluster_epoch{2});
...
TEST_F_CORO(ctp_stm_fixture, test_last_reconciled_offset) {
    ...
    auto gc_epoch_before = api(node(*get_leader())).get_min_epoch();
    ASSERT_TRUE_CORO(gc_epoch_before.has_value());
    ASSERT_EQ_CORO(gc_epoch_before.value(), ct::cluster_epoch{1});
    ...
    // Check that max and projected epochs remain the same
    auto max_epoch_after = api(node(*get_leader())).get_max_epoch();
    auto projected_epoch_after = api(node(*get_leader())).get_projected_epoch();
    ASSERT_TRUE_CORO(max_epoch_after.has_value());
    ASSERT_TRUE_CORO(projected_epoch_after.has_value());
    ASSERT_EQ_CORO(max_epoch_after.value(), ct::cluster_epoch{2});
    ASSERT_EQ_CORO(projected_epoch_after.value(), ct::cluster_epoch{2});

    // Check that first epoch to remove has moved forward
    auto gc_epoch_after = api(node(*get_leader())).get_min_epoch();
    ASSERT_TRUE_CORO(gc_epoch_after.has_value());
    ASSERT_EQ_CORO(gc_epoch_after.value(), ct::cluster_epoch{2});
    ...
    projected_epoch_after = api(node(*get_leader())).get_projected_epoch();
    ASSERT_TRUE_CORO(max_epoch_after.has_value());
    ASSERT_TRUE_CORO(projected_epoch_after.has_value());

    // We know that b2 started epoch 2 but we don't yet know where it ends
    gc_epoch_after = api(node(*get_leader())).get_min_epoch();
    ASSERT_TRUE_CORO(gc_epoch_after.has_value());
    ASSERT_EQ_CORO(gc_epoch_after.value(), ct::cluster_epoch{2});
}
...
TEST_F_CORO(ctp_stm_fixture, test_truncate_all_epochs) {
    ...
    ASSERT_EQ_CORO(projected_epoch.value(), last_epoch);

    // Nothing yet reconciled
    auto min_epoch = api(node(*get_leader())).get_min_epoch();
    ASSERT_TRUE_CORO(min_epoch.has_value());
    ASSERT_EQ_CORO(min_epoch.value(), ct::cluster_epoch{0});

    // Advance reconciled offset to the middle of the first epoch
    co_await api(node(*get_leader()))
      .advance_reconciled_offset(kafka::offset(5));
    min_epoch = api(node(*get_leader())).get_min_epoch();
    ASSERT_TRUE_CORO(min_epoch.has_value());
    ASSERT_EQ_CORO(min_epoch.value(), ct::cluster_epoch{0});

    // Advance reconciled offset exactly to the end of the first epoch
    co_await api(node(*get_leader()))
      .advance_reconciled_offset(kafka::offset(9));
    min_epoch = api(node(*get_leader())).get_min_epoch();
    ASSERT_TRUE_CORO(min_epoch.has_value());
    ASSERT_EQ_CORO(min_epoch.value(), ct::cluster_epoch{5});
}
