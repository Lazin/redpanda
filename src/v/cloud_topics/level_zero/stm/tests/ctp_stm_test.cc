...
TEST_F_CORO(ctp_stm_fixture, truncates_below_lro) {
    ...
    // The allowed local start offset is now the floor that drives prefix
    // truncation. Set it to the same kafka offset so the housekeeping fiber
    // has a reason to trim.
    auto floor_res = co_await replicate_set_allowed_local_start_offset(
...
TEST_F_CORO(ctp_stm_fixture, can_replay_truncated_log) {
    ...
    co_await leader_api.advance_reconciled_offset(
      kafka::offset{2000}, model::no_timeout, as);
    // The allowed local start offset drives prefix truncation; set it so
...
TEST_F_CORO(ctp_stm_fixture, prefix_truncate_target_no_signal_no_trim) {
    // Under floor semantics, an unset allowed local start offset + no
    ...
    // LRLO has advanced past min, but with no allowed local start offset and
...
TEST_F_CORO(
  ctp_stm_fixture,
  prefix_truncate_target_collapses_when_allowed_local_start_offset_cleared) {
    // Under floor semantics, clearing the allowed local start offset removes
    ...
    auto res = co_await replicate_set_allowed_local_start_offset(
      leader, kafka::offset{50});
    ASSERT_TRUE_CORO(res.has_value());
    // allowed_local_start_offset=50 is below LRLO, so it drives the target.
    auto expected = leader.raft()->log()->to_log_offset(
