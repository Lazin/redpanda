...
TEST(ctp_stm_state_test, initial_state) {
  ...
  EXPECT_FALSE(state.get_projected_epoch().has_value());
...
TEST(ctp_stm_state_test, advance_projected_epoch) {
  ct::ctp_stm_state state;
  ct::cluster_epoch epoch1(10);
  ct::cluster_epoch epoch2(20);
  ct::cluster_epoch epoch3(5);

  state.advance_projected_epoch(epoch1);
  EXPECT_EQ(state.get_projected_epoch().value(), epoch1);

  state.advance_projected_epoch(epoch2);
  EXPECT_EQ(state.get_projected_epoch().value(), epoch2);

  // Should not go backwards
  state.advance_projected_epoch(epoch3);
  EXPECT_EQ(state.get_projected_epoch().value(), epoch2);
}
...
TEST(ctp_stm_state_test, advance_epoch_on_a_follower) {
  // On a follower the projected epoch should also be updated
  ct::ctp_stm_state state;
  ct::cluster_epoch advance_epoch(20);

  state.advance_epoch(advance_epoch);

  EXPECT_EQ(state.get_projected_epoch().value(), advance_epoch);
