...
private:
    ...
    ///   retention_target = log->adjusted_retention_offset(gc_cfg)
    ///                      (or model::offset::max() under storage.mode=cloud)
    ///   floor            = max(retention_target, allowed_local_start_offset)
    ///   target           = min(max_removable_local_log_offset, floor)
    /// The allowed local start offset is the L1 compaction floor; the helper
    ...
    /// Build the gc_config to feed adjusted_retention_offset. Derives
