...
/// the record batch cache but independently (through the normal code path in storage layer).
/// This component stores materialized batches but the batch_cache_index in the segment of the underlying partition
/// stores placeholders.
class batch_cache {
public:
    ...
    // The code that uses this class should only use this to cache committed entries.
...
private:
    // NOTE: in the storage layer we have multiple indexes per partition (one per segment).
    // Here we only have one index per cloud storage partition. From what I see it should be
    // OK to use index this way. Likely even more efficient compared to index per segment.
