...
ss::future<> compaction_sink::finalize(bool success) {
    ...
    if (_any_object_failed) {
        ...
        // Compute the new allowed_local_start_offset floor for this
        ...
        if (max_cleaned_last_offset.has_value() && _notifier != nullptr) {
            ...
            vlog(
              _ctxlog.info,
              "[{}] compaction advancing allowed_local_start_offset to {}",
              _tp,
              new_floor);
            auto res = co_await _notifier->set_allowed_local_start_offset(
              _ntp, new_floor);
            if (!res.has_value()) {
                if (tombstones_removed_above_floor) {
                    vlog(
                      ...
                      "allowed_local_start_offset to {} ({}) while tombstones "
                ...
                vlog(
                  _ctxlog.warn,
                  "[{}] failed to advance allowed_local_start_offset to {} "
