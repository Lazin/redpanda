...
template<>
struct convert<model::cloud_storage_default_mode> {
    using type = model::cloud_storage_default_mode;

    static Node encode(const type& rhs) { return Node(fmt::format("{}", rhs)); }

    static bool decode(const Node& node, type& rhs) {
        auto value = node.as<std::string>();
        auto mode = model::cloud_storage_default_mode_from_string(value);
        if (!mode) {
            return false;
        }
        rhs = mode.value();
